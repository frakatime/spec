...
### Authentication
While authentication is enforced, the way you go about it is not. For example, in `ft-server-node` it's just two environment varibles, but if you were to develop a production-ready implementation you would want to build on top of Frakatime to provide endpoints for authentication.\
